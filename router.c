...
void process_receive_updates(int *neighbor_fds, int converge_fd) {
    ...
    for (i = 0; i < NumRoutes; i++)
    	if (neighborCost[i].nbr == sender_id) {
    		cost_to_sender = neighborCost[i].cost;
    		break;
    	}
  ...
  //  }
	...
	newCost = entry.cost + cost_to_sender;
	if(newCost > INFINITY){
		newCost = INFINITY;
	}
    	int found = 0;
...
void init_router(int argc, char **argv) {
	...
	neighborCost[i] = initResponse.nbrcost[i];
fprintf(log_file, "-------\n");
printTable();
fprintf(log_file, "--------\n");
}
